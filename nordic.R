...
library(ggplot2)
library(readxl)
library(thematic) # remotes::install_github("rstudio/thematic")
...
# All countries
wdall <- bind_rows(wdnor, wdswe, wddnk) %>%
  filter(year >= 2015,
         year < 2021 | week <= isoweek(Sys.Date()) - 2)
...
wdall %>%
  group_by(country, point_type, week) %>%
  summarize(deaths_per100k = mean(deaths_per100k)) %>%
  ggplot(aes(x = week, y = deaths_per100k,
                  color = country)) +
  geom_line(aes(linetype = point_type)) +
  guides(linetype = guide_legend(override.aes = list(color = 8))) +
